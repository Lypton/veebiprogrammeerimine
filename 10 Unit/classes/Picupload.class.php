<?php
	class Picupload {
		...
		private function checkImageForUpload(){
			...
			//Kui kõik sobib, teeb vajaliku pildiobjekti.
			if($this->error == null){
		...
		public function resizeImage($maxPicW, $maxPicH){
		    ...
		    if($imageW > $maxPicW or $imageH > $maxPicH){
		    	...
		    	//Kui pole piisavalt suur, siis jätab originaalsuuruse
		...
		private function setPicSize($myTempImage, $imageW, $imageH, $imageNewW, $imageNewH){
			...
			$cutW = $imageW;
			$cutH = $imageH;
			//Kärpimine ruudukujulisest pildist (kui on ruut)
			if($imageNewW == $imageNewH){
				//Kui pilt on "Landscape" kujul.
				if($imageW > $imageH){
