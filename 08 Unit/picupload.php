<?php
  ...
  if(isset($_POST["submitPic"])) {
    ...
    //Eripärase nime lisamine pildi üleslaadimisel
    $filename = "vp_";
  ...
  // if everything is ok, try to upload file
    ...
    if($imageW > $maxPicW or $imageH > $maxPicH){
      ...
      //-JPG-//
      if($imageFileType == "jpg" or $imageFileType == "jpeg"){
        if(imagejpeg($myNewImage, $pic_upload_dir_orig . $filename, 90)){
      ...
      //-PNG-//
      if($imageFileType == "png"){
        if(imagepng($myNewImage, $pic_upload_dir_orig . $filename, 6)){
          $notice = "Vähendatud faili salvestamine õnnestus!";
        } else {
          $notice = "Vähendatud faili salvestamine ei õnnestunud.";
        }
      ...
      //-GIF-//
      if($imageFileType == "gif"){
        if(imagegif($myNewImage, $pic_upload_dir_orig . $filename)){
          $notice = "Vähendatud faili salvestamine õnnestus!";
        } else {
          $notice = "Vähendatud faili salvestamine ei õnnestunud.";
        }
...
?>
  ...
  <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data">
    <label>Valis üleslaetav pildifail</label><br>
