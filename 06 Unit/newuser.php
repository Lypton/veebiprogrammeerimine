<?php
  ...
  //testtsükkel
  // $i = $i +1     $i += 1       $i ++         HARJUTAMISEKS.
  //for($i = 0; $i < 10; $i ++){
    //echo "Tsükkel töötab " .$i ." I ";
  //}
  ...
  if(isset($_POST["submitUserData"])) {
    ...
    if(isset($_POST["surname"]) and !empty($_POST["surname"])){
      ...
      $nameError = "Palun sisesta perekonnanimi!";
    }
  ...
  if(!empty($birthDay) and !empty($birthMonth) and !empty($birthYear)){
    if(checkdate($birthMonth, $birthDay, $birthYear)){
      ...
      $birthDateError = "Kahjuks antud kuupäev pole valiidne."; 
    }//if checkdate
  }
    //email kontroll
  if(isset($_POST["email"]) and !empty($_POST["email"])){
    ...
    //parooli kontroll (kas on olemas, kas vähemalt 8 tähte ning kaks korda ühtemoodi)
  if(!isset($_POST["password"]) and !empty($_POST["password"])){
    $passwordError = "Palun sisesta salasõna!";
  } else {
      if(strlen($_POST["password"]) < 8){
        $passwordError = "Liiga lühike salasõna (sisestasite vaid " .strlen($_POST["password"]) ." märki).";
    }
  }

  if(!isset($_POST["confirmpassword"]) or empty($_POST["confirmpassword"])){
    $passwordError = "Palun sisesta salasõna ka teist korda!";
  } else {
